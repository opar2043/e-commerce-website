...
const Card = ({ product }) => {
  const { _id, name, images, weight, category, shortDescription, isAvailable } =
    product;
  ...
  const getGoldPurity = () => {
    const categoryLower = category?.toLowerCase() || "";
    if (categoryLower.includes("22k") || categoryLower.includes("gold"))
      return "22K";
    ...
    <Link to={`/product/${_id}`}>
      <div className="group relative bg-white border-4 border-[#C7A94D] rounded-lg shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:scale-105 overflow-hidden ">
        ...
        <div className="p-6 bg-white">
          {/* Product Name */}
          <motion.h3 className="font-bold text-xl text-gray-800 mb-2 line-clamp-2 group-hover:text-amber-600 transition-colors">
            {name}
          </motion.h3>
          ...
          {shortDescription && (
            <motion.p
              ...
              className="text-gray-600 text-md mb-3 line-clamp-2"
            >
              {shortDescription}
            </motion.p>
          )}
