...
import banner from "../../assets/gold2.webp";
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row-reverse gap-6 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="lg:w-1/2 grid grid-cols-1 md:grid-cols-2 gap-6">
        {blogs.map((blog) => (
          <div key={blog.id} className="bg-white  overflow-hidden transition-transform duration-300  hover:-translate-y-1">
            <img src={blog.image} alt={blog.name} className="w-full h-48 object-cover" />
            <div className="p-5">
              ...
              <h3 className="text-lg font-semibold mb-4 text-[#191919]">{blog.name}</h3>
              <p className="text-gray-700 text-sm mb-6">{blog.blog.substring(0, 80)}...</p>
              <button className="px-4 py-2 border border-gray-800 text-gray-800 font-medium text-sm hover:bg-gray-800 hover:text-white transition-all duration-300 flex items-center">
      ...
      <div className="lg:w-1/2 relative flex items-center justify-center  overflow-hidden">
        <img src={banner} alt="Gold Banner" className="w-full h-full object-cover absolute inset-0" />
        <div className="absolute inset-0 bg-black bg-opacity-40"></div>
        <div className="relative z-10 text-center text-white p-8 max-w-md">
          <h2 className="text-6xl font-serif mb-2 italic">Anna Style</h2>
